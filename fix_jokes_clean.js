...
const targetWasBlock = \`Target was {currentLogo.name}</div>
                      </div>\`;

const targetWasBlockNew = \`Target was {currentLogo.name}</div>
                      {jokeContent && (
                        <div className="flex items-center gap-3 bg-white px-6 py-4 border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] rounded-none text-center mx-4 mt-4">
...
code = code.replace(targetWasBlock, targetWasBlockNew);

// Ensure handleTimeOut sets jokeContent
const handleTimeOutOld = \`  const handleTimeOut = () => {
    setGameStatus('wrong');
...
const handleTimeOutNew = \`  const handleTimeOut = () => {
    setGameStatus('wrong');
...
if (code.includes(handleTimeOutOld)) {
    code = code.replace(handleTimeOutOld, handleTimeOutNew);
} else {
    // try with windows line breaks
    code = code.replace(handleTimeOutOld.replace(/\\n/g, '\\r\\n'), handleTimeOutNew.replace(/\\n/g, '\\r\\n'));
}
